    ...
    <ScrollView style={{ flex: 1, backgroundColor: "black" }}>
      {Data.map((product,index) => (
        ...
        <TouchableOpacity onPress={() => {handleAdd(product)}}>
            {product.image}
            <Text style={{ color: "white", fontSize: 15, fontFamily: "JosefinSans_600SemiBold_Italic", textAlign: "center" }}>{product.title}</Text>
            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
            <Text style={{textAlignVertical:"center",color:"#A2A2A1FF"}}>Add To Cart</Text>
              <Text style={styles.forward1}><MaterialCommunityIcons name='cart' size={27} color="#A2A2A1FF" />{ }</Text>
            </View>
        </TouchableOpacity>
          </View>
